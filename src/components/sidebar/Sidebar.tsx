    ...
    <>
        <Box width={{xs: '100%', md: '30%'}} >
            <Box position={'sticky'} top={'100px'} sx={{ transition: 'all .3s ease' }}>
                    ...
                    <Box sx={{display: 'flex', flexDirection: 'column', marginTop: '20px'}}>
                        {latestBlogs.map(item => (
                            <Fragment key={item.title}>
                                <Box onClick={() => router.push(`/blog/${item.slug}`)}  sx={{ display: 'flex', gap: '20px', alignItems: 'center', marginTop: '20px', cursor: 'pointer' }}>
                ...
                <Box padding={"20px"} marginTop={'20px'} border={'1px solid grey'} borderRadius={'8px'} >
